...
def recursion(start, w, cnt, n):
    ...
    inDegree[start] -= 1
    if inDegree[start] < -1 or len(order[start]) == 0:
        time_list[start] = -1
        return -1

    x = -1
    for i in range(len(order[start])):
        y = recursion(order[start][i], w, cnt + 1, n)
        if x < y:
            x = y
    if x == -1:
        time_list[start] = -1
...
for i in range(t):
    ...
    for j in range(k):
        ...
        order[x].append(y)
        inDegree[y] += 1
    #w : building number that construction is needed to win
    w = int(stdin.readline())
    time_list = [0 for i in range(n + 1)]  #w까지 건설하는데 드는 시간 저장
    ans = -1
    for d in range(1, n+1):
        if inDegree[d] == 0:
            result = recursion(d, w, 0, n)
            if ans < result:
                ans = result
    print(ans)
